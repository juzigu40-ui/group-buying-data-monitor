...
def build_signal_input_readme(store_name: str, city: str) -> str:
    lines = [
        ...
        "- 扩展：`author_level`, `verified_label`, `follower_count`, `author_tags`, `ip_location`, `topic_tags`, `favorite_count`",
        "",
        "## 平台字段重点",
        "",
        "### 小红书",
        ...
        "",
        "### 抖音",
        "- 需要重点提供：发布者级别、认证信息、粉丝量、账号标签、标题、视频文案、点赞、评论、转发、POI 门店地址。",
        "",
        "### 视频号",
        "- 需要重点提供：标题、视频文案、认证信息、粉丝量、账号标签、点赞、转发、POI 门店地址。",
        "",
        "## 最小 JSON 示例",
        "",
        "```json",
...
def build_profile_deliverable(profile_dir: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "- 按门店统计每天相关内容条数，并和门店目标值做达标对比。",
            "- 如果内容来自门店碰一碰 / 内容库 / 活动台账，系统可以按门店来源字段直接记账，用来看各门店当天内容 KPI 是否达标。",
            ...
            "- 推荐交付模式：先按托管代跑 + 飞书推送交付，让老板先在飞书上做决策；客户后续如果确实要自跑，再补完整技术运行包。",
...
def build_profile_usage_guide(profile_dir: Path) -> str:
    ...
    lines = [
        ...
        "",
        f"`./scripts/run_acceptance_demo.sh {profile_path} --require-feishu`",
        "",
        "这条命令会自动做 3 件事：",
        "",
        ...
        f"2. 先做运行前自检：`PYTHONPATH=src python3 -m gb_monitor.cli profile-readiness --profile-dir '{profile_path}' --require-feishu`。",
        "3. 再测试飞书机器人连通性：`PYTHONPATH=src python3 -m gb_monitor.cli feishu-ping`。",
        f"4. 跑整店链路：`./scripts/run_profile.sh {profile_path}`。如果 `.env` 已配置 webhook，这条命令会自动把运行摘要和实时舆情结果推到飞书。",
        f"5. 只跑实时舆情正式增量：`PYTHONPATH=src python3 -m gb_monitor.cli profile-signals --profile-dir '{profile_path}' --mode all --notify --mark-dispatched --attribution-window-hours 2 --board-output '{profile_path}/signal_watchboard.md' --report-output '{profile_path}/signal_report.txt'`",
        f"6. 导出客户展示版完整快照：`PYTHONPATH=src python3 -m gb_monitor.cli profile-signals --profile-dir '{profile_path}' --mode all --dedupe-hours 0 --attribution-window-hours 2 --board-output '{profile_path}/signal_watchboard.md' --report-output '{profile_path}/signal_report.txt'`",
        f"7. 生成舆情说明页：`PYTHONPATH=src python3 -m gb_monitor.cli profile-signal-deliverable --profile-dir '{profile_path}' --output '{profile_path}/signal_delivery_explainer.md'`",
        f"8. 生成客户使用说明：`PYTHONPATH=src python3 -m gb_monitor.cli profile-usage-guide --profile-dir '{profile_path}' --output '{profile_path}/client_usage_guide.md'`",
        "",
        "## 重点达人怎么配置",
        "",
        f"- 打开 `{profile_path}/store_signal_rules.json`。",
