...
def build_profile_usage_guide(profile_dir: Path) -> str:
    ...
    lines = [
        ...
        f"3. 跑整店链路：`./scripts/run_profile.sh {profile_path}`。如果 `.env` 已配置 webhook，这条命令会自动把运行摘要和实时舆情结果推到飞书。",
        f"4. 只跑实时舆情正式增量：`PYTHONPATH=src python3 -m gb_monitor.cli profile-signals --profile-dir '{profile_path}' --mode all --notify --mark-dispatched --attribution-window-hours 2 --board-output '{profile_path}/signal_watchboard.md' --report-output '{profile_path}/signal_report.txt'`",
        f"5. 导出客户展示版完整快照：`PYTHONPATH=src python3 -m gb_monitor.cli profile-signals --profile-dir '{profile_path}' --mode all --dedupe-hours 0 --attribution-window-hours 2 --board-output '{profile_path}/signal_watchboard.md' --report-output '{profile_path}/signal_report.txt'`",
        f"6. 生成舆情说明页：`PYTHONPATH=src python3 -m gb_monitor.cli profile-signal-deliverable --profile-dir '{profile_path}' --output '{profile_path}/signal_delivery_explainer.md'`",
        f"7. 生成客户使用说明：`PYTHONPATH=src python3 -m gb_monitor.cli profile-usage-guide --profile-dir '{profile_path}' --output '{profile_path}/client_usage_guide.md'`",
        "",
        "## 重点达人怎么配置",
        "",
