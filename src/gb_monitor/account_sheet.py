...
def build_profile_signal_deliverable(profile_dir: Path) -> str:
    ...
    lines = [
        ...
        "7. 派发层可直接生成 `signal_report.txt` 和 `signal_watchboard.md`，也可以接飞书 webhook。",
        "",
        "## 与需求文档的对应关系",
        "",
...
def build_profile_usage_guide(profile_dir: Path) -> str:
    ...
    lines = [
        ...
        f"2. 只跑实时舆情正式增量：`PYTHONPATH=src python3 -m gb_monitor.cli profile-signals --profile-dir '{profile_path}' --mode all --mark-dispatched --attribution-window-hours 2 --board-output '{profile_path}/signal_watchboard.md' --report-output '{profile_path}/signal_report.txt'`",
        f"3. 导出客户展示版完整快照：`PYTHONPATH=src python3 -m gb_monitor.cli profile-signals --profile-dir '{profile_path}' --mode all --dedupe-hours 0 --attribution-window-hours 2 --board-output '{profile_path}/signal_watchboard.md' --report-output '{profile_path}/signal_report.txt'`",
        f"4. 生成舆情说明页：`PYTHONPATH=src python3 -m gb_monitor.cli profile-signal-deliverable --profile-dir '{profile_path}' --output '{profile_path}/signal_delivery_explainer.md'`",
        f"5. 生成客户使用说明：`PYTHONPATH=src python3 -m gb_monitor.cli profile-usage-guide --profile-dir '{profile_path}' --output '{profile_path}/client_usage_guide.md'`",
        "",
        "## 重点达人怎么配置",
        "",
